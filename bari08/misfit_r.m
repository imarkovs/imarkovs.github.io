% MISFIT - misfit computation using image representation
%          (SISO case only, efficient implementation)

function [m,wh] = misfit(w,r)
...
p  = -r(2:2:end);
lp = length(p); l = lp - 1;
Pf = [fliplr(p); [fliplr(q) zeros(lp-length(q))]];
P  = fliplr(Pf);
Pv = Pf(:)';

% r = A'*w, A = blktoep([zeros(2,T-1) P zeros(2,T-1)]',T);
Xp = blktoep([zeros(2,l-1) P(:,1:l)]',l)';   % past induced state map
Xf = blktoep([P(:,2:l+1) zeros(2,l-1)]',l)'; % future induced state map

wi = w(1:l,:)';
ri = Xp * wi(:); % xp(P,w(1:lp-1,:));

wf = w(end-lp+2:end,:)';
rf = Xf * wf(:); % xf(P,w(end-lp+2:end,:));
