...
\label{sec:org8aefbe4}
The following subbehaviors of \(\mathcal{B}\) are of special interest:
\begin{itemize}
\item \(\mathcal{Y}_0\) --- \emph{zero-input subbehavior}, i.e., the set of transient responses,
\item \(\mathcal{U}_0\) --- \emph{zero-output subbehavior}, i.e., the set of inputs blocked by the system,
\item \(\mathcal{B}_0\)          --- \emph{zero initial conditions subbehavior}, i.e., the set of zero initial conditions trajectories,
\item \(\mathcal{B}_\text{c}\)   --- \emph{controllable subbehavior}, i.e., the set of trajectories that are patchable with zero past trajectory, and
\item \(\mathcal{B}_\text{p}\)   --- \emph{periodic subbehavior}, i.e., the set of periodic trajectories.
\end{itemize}
...
from which \(m=\mathbf{m}(\mathcal{B})\) and \(n=\mathbf{n}(\mathcal{B})\) can be found. The resulting method is implemented in the function \texttt{BT2c}:
\begin{minted}[]{matlab}
...
\texttt{BT2c} finds \(\dim\,\mathcal{B}|_{T-1}\) and \(\dim\,\mathcal{B}|_{T}\) by rank computation, i.e., thresholding of the singular values using \danger a user defined threshold \texttt{tol}. 

\subsubsection*{Distance between systems}
\label{sec:orgf372188}
...
\label{sec:orgfe9a334}

The controllable subbehavior \(\mathcal{B}_c\subset\mathcal{B}|_T\) for \(T\geq\mathbf{\ell}(\mathcal{B})\) is \(mT+n\)-dimensional if and only if \(\mathcal{B}\) is controllable. This leads to a data-driven controllability test that is implemented in the function \texttt{isunctr}:
\begin{minted}[]{matlab}
...
R = B2R(B); R = BT2R(BT, q); 
\end{minted}
The inverse operation---finding the restricted behavior from a kernel or a state-space representation---is done by the functions \texttt{R2BT} and \texttt{ss2BT}, which implement the model-based approach, i.e., they construct \(B_T\) from the model parameters \(R\) and \((A,B,C,D)\), respectively. 
\begin{minted}[]{matlab}
...
\label{sec:org67cfaa4}

The basic operation of projection of a signal \(w\in(\mathbb{R}^q)^T\) on the behavior \(\mathcal{B}|_T\) of a system \(\mathcal{B}\in\mathcal{L}^q\) is equivalent to computing the \emph{distance from \(w\) to \(\mathcal{B}\)}. The same operation is equivalent to (errors-in-variables) Kalman smoothing: the projection \(\hat w\) is the \emph{smoothed version} of \(w\). The problem is solved by the function \texttt{dist}:
\begin{minted}[]{matlab}
[d, wh] = dist(w, B);
\end{minted}
...
\label{sec:orgcbc5daf}

For \({w_\text{d}}\in\mathcal{B}|_T\), there is a corresponding initial conditions. 
If \(\mathcal{B}\) is defined by an input/state/output representation, the initial conditions can be specified by the initial state \(x_\text{ini} = x(1)\). The initial state \(x_\text{ini}\) corresponding to a trajectory \(w\) can be found with the function \texttt{w2xini}:
\begin{minted}[]{matlab}
...
The initial conditions can be specified in a representation free way by an initial trajectory \(w_\text{ini}\) of length \(T_{\text{ini}}\geq\mathbf{\ell}(\mathcal{B})\). An initial trajectory \(w_\text{ini}\) of corresponding to a given trajectory \(w\) can be computed by the function \texttt{w2wini}:
\begin{minted}[]{matlab}
...
\end{minted}
The input parameters \texttt{v}, \texttt{wp}, and \texttt{wf} are optional. The default value of \texttt{v} is \texttt{ones(Tr, q)}, i.e., uniform weights for all variables and all moments of time. If \texttt{wp} and/or \texttt{wf} is not specified, then the corresponding constraints are dropped, i.e., the corresponding initial or final condition is free.
...
Td = 100; wd = B2w(B, Td); 
\end{minted}
A necessary and sufficient condition for \({w_\text{d}}\) to define \(\mathcal{B}\in\mathcal{L}^q_c\) is the \emph{identifiability condition}:
$\backslash$]
\text{rank} $\backslash$, \mathcal{H}\textsubscript{\mathbf{\ell}(\mathcal{B})+1}(\{w\textsubscript{\text}\{d\}\}) = \mathbf{m}(\mathcal{B})\big(\mathbf{\ell}(\mathcal{B})+1\big) + \mathbf{n}(\mathcal{B}).
$\backslash$]
This is a \emph{generalized persistency of excitation} condition on the data \({w_\text{d}}\) and requires prior knowledge of the complexity \(\mathbf{c}(\mathcal{B})\) of the data-generating system \(\mathcal{B}\). 
...
$\backslash$]
\text{rank} $\backslash$, \mathcal{H}\textsubscript{T}(\{w\textsubscript{\text}\{d\}\}) = \mathbf{m}(\mathcal{B})T + \mathbf{n}(\mathcal{B}),
$\backslash$]
...
$\backslash$]
T\textsubscript{\text{d}} \(\ge\) T\textsubscript{\(\min\)} := (m+1)T+n-1,
$\backslash$]
\begin{minted}[]{matlab}
...
An analog of \texttt{BT2c} for complexity computation from data \({w_\text{d}}\) is \texttt{c\_mpum}, which computes \(\mathbf{c}\big(\mathcal{B}_\text{mpum}({w_\text{d}})\big)\) without explicitly identifying \(\mathcal{B}_\text{mpum}({w_\text{d}})\):
\begin{minted}[]{matlab}
ch = c_mpum(wd); check(all(ch == [m, lag(B), n]))
\end{minted}
The function \texttt{w2BT} computes a basis \(B_T\) for \(\mathcal{B}_\text{mpum}({w_\text{d}})|_T\) from data \({w_\text{d}}\) 
\begin{minted}[]{matlab}
BhT = w2BT(wd, T);
\end{minted}
...
The following functions compute parametric representations of \(\mathcal{B}_\text{mpum}({w_\text{d}})\) and thus by construction impose both the bounded complexity and LTI structure. A kernel representation of \(\mathcal{B}_\text{mpum}({w_\text{d}})\) is obtained by the function \texttt{w2R}:
\begin{minted}[]{matlab}
...
\end{minted}
A lag recursive method for computing a \emph{minimal} kernel representation of \(\mathcal{B}_\text{mpum}({w_\text{d}})\) is implemented in the function \texttt{mpum}:
\begin{minted}[]{matlab}
