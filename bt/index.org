...
** Finding the number of inputs and the order

The number of inputs and the order are properties of the system. Their importance is due to the fact that they determine the system's complexity: The more variables are inputs and the higher the order is, the more complex the system is. 

The function =w2c= of the toolbox computes the number of inputs and the order directly from data. Here is an example: 
...
In general, a system with inputs (called an open system) admits multiple input/output partitionings. The function =BT2IO= finds all valid input/output partitionings for a system specified by an non-parametric representation of the finite-horizon behavior. This representation is the main one used in the toolbox and can be computed from given data or from another representation of the system. 

Continuing the example above, first, we obtain the non-parametric representation of the data-generating system from the given data using the function =w2BT=: 
#+begin_src matlab :tangle examples.m
T = 2 * ell + 1; BT = w2BT(wd, T, c);
#+end_src
The horizon $T$ should be chosen "sufficiently" large for the problem at hand. For find input/output partitionings of the variables, the horizon should be at least $2\ell+1$, where $\ell$ is the lag the system---another property of the system related to its complexity. Since the lag was already computed by the function =w2c= in the previous example, we reuse it here. 

Once the representation =BT= is computed, we call =BT2IO= for finding all valid input/output partitionings of the variables:
#+begin_src matlab :tangle examples.m
IO = BT2IO(BT, m + p)
#+end_src
Its output =IO= is a matrix, the rows of which indicate the valid input/output partitionings as follows: has as first $m$ variables =IO(:, 1:m)= indicate the indeces of the input variables and the remaining $p$ variables =IO(:, m+1:end)= indicate the indeces of the output variables. For example,
#+begin_src matlab :tangle examples.m
...
** Distance of a signal to a system                                     :NP:

** Distance between systems                                             :NP:
